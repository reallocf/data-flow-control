"""TPC-H Q01 policy OR-chain strategy for DFC vs Logical overhead."""
...
def build_tpch_q01_or_policy(or_count: int) -> DFCPolicy:
    """Build a single policy with an expanding OR predicate."""
    if or_count < 0:
        raise ValueError("or_count must be non-negative")

    clauses = ["max(lineitem.l_quantity) >= 0"]
    for i in range(or_count):
        col = _OR_COLUMNS[i % len(_OR_COLUMNS)]
        clauses.append(f"max({col} + {i + 1}) >= 0")

    constraint = " OR ".join(clauses)
    return DFCPolicy(
        sources=["lineitem"],
        constraint=constraint,
        on_fail=Resolution.REMOVE,
        description=f"q01_or_count_{or_count}",
    )


class TPCHPolicyManyORsStrategy(ExperimentStrategy):
    """Strategy for measuring performance vs number of OR clauses on TPC-H Q01."""

    def setup(self, context: ExperimentContext) -> None:
        self.scale_factor = float(context.strategy_config.get("tpch_sf", 1))
        db_path = context.strategy_config.get("tpch_db_path")
        if not db_path:
            db_path = f"./results/tpch_q01_policy_many_ors_sf{self.scale_factor}.db"
    ...
    def execute(self, context: ExperimentContext) -> ExperimentResult:
        ...
        policy = build_tpch_q01_or_policy(or_count)
        policies = [policy]
