...
impl SelectAnalysis {
    ...
    pub(crate) fn from_select_with_id(
        select: &Select,
        scope_id: u32,
        ...
        Self {
            ...
            is_aggregation: select_is_aggregation(select, registry),
...
impl StatementAnalysis {
    pub fn from_statement(statement: &Statement) -> Self {
        Self::from_statement_with_registry(
            statement,
            &AggregateRegistry::for_dialect(SqlDialect::DuckDb),
            None,
        )
    ...
    pub(crate) fn from_statement_with_registry(
        statement: &Statement,
        registry: &AggregateRegistry,
        ...
        collect_select_scopes(statement, registry, &mut analysis.select_scopes, 0);
        if let Some(stats) = stats {
...
fn collect_select_scopes(
    ...
    match statement {
        Statement::Query(query) => collect_query_scopes(query, registry, scopes, next_id),
        Statement::Insert(insert) => {
            if let Some(source) = &insert.source {
                collect_query_scopes(source, registry, scopes, next_id)
            } else {
                next_id
            }
        }
        ...
        Statement::Merge {
            ...
            ..
        } => collect_query_scopes(subquery, registry, scopes, next_id),
        _ => next_id,
    }
}

fn collect_query_scopes(
    query: &Query,
    registry: &AggregateRegistry,
    scopes: &mut Vec<SelectAnalysis>,
    mut next_id: u32,
    ...
    if let Some(with) = &query.with {
        for cte in &with.cte_tables {
            next_id = collect_query_scopes(&cte.query, registry, scopes, next_id);
        }
    }
    collect_set_expr_scopes(query.body.as_ref(), registry, scopes, next_id)
}
...
fn collect_set_expr_scopes(
    ...
    match set_expr {
        SetExpr::Select(select) => {
            let id = next_id;
            scopes.push(SelectAnalysis::from_select_with_id(select, id, registry));
            collect_select_nested_scopes(select, registry, scopes, id + 1)
        }
        SetExpr::Query(query) => collect_query_scopes(query, registry, scopes, next_id),
        SetExpr::SetOperation { left, right, .. } => {
            let next_id = collect_set_expr_scopes(left, registry, scopes, next_id);
            collect_set_expr_scopes(right, registry, scopes, next_id)
        }
        _ => next_id,
    }
}
...
fn collect_select_nested_scopes(
    ...
    for table in &select.from {
        next_id = collect_table_factor_scopes(&table.relation, registry, scopes, next_id);
        for join in &table.joins {
            next_id = collect_table_factor_scopes(&join.relation, registry, scopes, next_id);
        }
    }
    next_id = collect_expr_scopes(select.selection.as_ref(), registry, scopes, next_id);
    next_id = collect_expr_scopes(select.having.as_ref(), registry, scopes, next_id);
    for item in &select.projection {
        if let sqlparser::ast::SelectItem::UnnamedExpr(expr)
        | sqlparser::ast::SelectItem::ExprWithAlias { expr, .. } = item
        {
            next_id = collect_expr_scopes(Some(expr), registry, scopes, next_id);
        }
    }
    next_id
}
...
fn collect_expr_scopes(
    ...
    walk_expr_subqueries(expr, registry, scopes, next_id)
}
...
fn walk_expr_subqueries(
    ...
    match expr {
        ...
        | sqlparser::ast::Expr::InSubquery {
            subquery: query, ..
        } => collect_query_scopes(query, registry, scopes, next_id),
        sqlparser::ast::Expr::BinaryOp { left, right, .. } => {
            let next_id = walk_expr_subqueries(left, registry, scopes, next_id);
            walk_expr_subqueries(right, registry, scopes, next_id)
        }
        ...
        | sqlparser::ast::Expr::IsNotNull(inner) => {
            walk_expr_subqueries(inner, registry, scopes, next_id)
        }
        sqlparser::ast::Expr::Function(function) => {
            ...
            if let sqlparser::ast::FunctionArguments::List(args) = &function.args {
                for arg in &args.args {
                    ...
                    {
                        next_id = walk_expr_subqueries(inner, registry, scopes, next_id);
                    }
                }
            }
            if let Some(filter) = function.filter.as_ref() {
                next_id = walk_expr_subqueries(filter, registry, scopes, next_id);
            }
            next_id
        }
        _ => next_id,
