...
sections_path = Path(os.getenv("INPUT", base / "inputs" / "title26_sections.jsonl"))
output_path = Path(os.getenv("OUTPUT", base / "outputs" / f"llm_reference_audit_{section}.csv"))
...
def prompt_for(piece, number):
    return f"""
Return valid JSON and no explanatory text.

Find legal cross-reference expressions in this excerpt from 26 U.S.C. section {number}.

Include exact phrases such as:
- section 162
- section 212
- section 274
- subsection (a)
...
For references inside section {number}, set:
...
def main():
    ...
    for chunk_start, piece in chunks(text):
        data = complete_json(prompt_for(piece, section), max_tokens=16000)
        ...
        for item in data.get("references", []):
            ...
            ref_class = normalize_ref_class(item.get("ref_class"))
    ...
    rows.sort(key=lambda row: int(row["span_start"]) if str(row["span_start"]).lstrip("-").isdigit() else 999999999)
